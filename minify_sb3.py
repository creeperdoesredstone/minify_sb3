...
* warp      "true"/"false" -> true/false  sequencer.js and irgen.js accept both forms
            (only applied to mutation.warp)
* compact   no whitespace and escape characters
* ids       Enable --rename-block-ids to give blocks sequential shortest IDs from the
            Scratch VM character pool, updating every block reference.
            WARNING: may increase file size.
* comments  remove every comment from every non-Stage target (sprites), i.e. clear
            target.comments to {} and delete each block's "comment" link.
...
* monitors  
            ...
            - delete monitors that are hidden and have no block that could show them
              and that were never moved/resized, or that point at a variable/list that no longer exists.
...
* lists     detect large lists (>= 4096 bytes of JSON / >= 1000 items by default)
            ...
            Tune with --list-bytes=N and --list-items=N.

Additional opt-in transforms:
...
* fields: {} / inputs: {}  the VM loader tolerates their absence, but sb3fix
                           (and other strict validators) throw "fields is not an
                           object" / "inputs is not an object". They cost ~280 KB
                           to keep, which is not worth breaking validators.

* next / parent : null     VM uses strict `!== null` (getTopLevelScript, deleteBlock)
* mutation.tagName/children  Blocks.mutationToXML() dereferences both when the
                           project is opened in the block editor
* costume/sound md5ext     the sound loader has no fallback (md5: soundSource.md5ext),
                           so dropping it from sounds silently breaks audio
...
After writing, the tool reloads both archives, re-inflates the omitted defaults
and checks the two projects are identical, and that every other zip entry is
